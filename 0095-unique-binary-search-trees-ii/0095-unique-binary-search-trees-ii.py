...
class Solution:
    ...
    def helper(self, start, end):
        ...
        for curRootVal in range(start, end + 1): # generate all roots using list [start, end]
            ...
            left_subtrees = self.helper(start, curRootVal - 1)
			
			# recursively get list of subtrees greater than curRoot 
            # (a BST must have right subtrees greater than the root)
            right_subtrees = self.helper(curRootVal + 1, end) 
			
            for left_subtree in left_subtrees:   # get each possible left subtree
                for right_subtree in right_subtrees: # get each possible right subtree
